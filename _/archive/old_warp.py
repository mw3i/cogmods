'''
WARP: MultiLayer Classifier (aka, multilayer perceptron) w/ WARP (exponential) activation function
    - Dr. Ken Kurtz, Dan Silliman
- - - - - - - - - - - - - - - - - - - - - - - - - - - 
...
--- Notes ---
    ...
    - uses exponential activation function on the hidden layer
    - linear outputs
'''
...
if __name__ == '__main__':
    ...
    hps = {
        ...
        'hidden_activation': lambda x: np.exp(x - inputs.shape[1]), # <-- warp activation function
        'hidden_activation_deriv': lambda x: np.exp(x - inputs.shape[1]), # <-- warp derivative

        'output_activation': lambda x: x, # <-- linear output
