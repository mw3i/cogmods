...
--- Notes ---
    ...
    - requires user to provide activation function & derivative (or use a tool like autograd to get the derivative)
    - backprop verified against 'autograd' automatic differentiation package (https://github.com/HIPS/autograd)
'''
...
def loss_grad(params, inputs, targets, hps):
    ...
    encode_grad = np.multiply(
        hps['hidden_activation_d'](hidden_act_raw),
        np.matmul(
            decode_grad, 
...
if __name__ == '__main__':
    ...
    hps = {
        ...
        'hidden_activation_d': lambda x: sigmoid(x) * (1 - sigmoid(x)),

        'output_activation': lambda x: x,
        'output_activation_d': lambda x: 1,
    }
    ...
    f = fit(params, inputs, one_hot_targets, hps, training_epochs = num_training_epochs)
